# AAMAS2023 竞赛四人麻将赛道 RL 自博弈 learner 部分
# author: fanzl
# date: 2023/4/14
# version: 1.1
# 版本改进：
...
from ResnetModel import Resnet18, orthogonal_init
...
class Learner:
    def __init__(self, net_lr):
        ...
        self.batch_size = 6000 * 32
        self.clip_range = 0.5
        self.SumWriter = SummaryWriter(log_dir="./save/tb")
        self.tb_x = 0
    
    def _calc_policy_loss(self, sample_action, old_prob, prob, sampled_advantage, clip_range=0.5):
        ...
        self.SumWriter.add_scalar('up clip rate', sum(ratio > 1 + self.clip_range).item()/self.batch_size, self.tb_x)
        self.SumWriter.add_scalar('down clip rate', sum(ratio < 1 - self.clip_range).item()/self.batch_size, self.tb_x)
        ...
        surr2 = torch.clamp(ratio, 1.0 - clip_range, 1.0 + clip_range) * norm_adv
        policy_loss = torch.mean(torch.min(surr1, surr2))
        return policy_loss
    
    def _calc_value_loss(self, value, old_value, sampled_advantage, clip_range=0.5):
        clipped_value = old_value + torch.clamp(value - old_value, - clip_range, clip_range)
    ...
    def update(self, transition_dict):
        ...
        for i in range(32):
            states = torch.tensor(transition_dict["states"][i*6000:(i+1)*6000], dtype=torch.float).to(self.device)                      # (4096, 145, 4, 9)
            actions = torch.tensor(transition_dict["actions"][i*6000:(i+1)*6000]).long().view(-1, 1).to(self.device)                    # (4096, 1)
            advantages = torch.tensor(transition_dict["advantages"][i*6000:(i+1)*6000], dtype=torch.float).view(-1, 1).to(self.device)  # (4096, 1)
            probs_old = torch.tensor(transition_dict["probs_old"][i*6000:(i+1)*6000], dtype=torch.float).view(-1, 1).to(self.device)    # (4096, 1)
            value_old = torch.tensor(transition_dict["value_old"][i*6000:(i+1)*6000], dtype=torch.float).view(-1, 1).to(self.device)    # (4096, 1)
            legal_actions = torch.tensor(transition_dict["legal_actions"][i*6000:(i+1)*6000], dtype=torch.float).to(self.device)        # (4096, 235)
...
def main():
    ...
    manager.start()
    mem_pool = manager.MemPool(200000)
