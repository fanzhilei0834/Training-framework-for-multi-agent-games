...
def my_controller(observation, action_space, is_act_continuous=False):
    # print("observation: ", observation)              # len(observation['obs']['action_mask']) == 38
    # print("controlled_player_index: ", observation['controlled_player_index'], "current_move_player: ", observation['current_move_player'])
    # print("observation: ", observation['obs']['observation'].shape if observation['obs'] else None)
    # print("action_mask: ", observation['obs']['action_mask'].shape if observation['obs'] else None)
    # print("action_space: ", action_space)            # [Discrete(38)]
    # print("is_act_continuous: ", is_act_continuous)  # False
    agent_action = []
    for i in range(len(action_space)):
        action_ = sample_single_dim(action_space[i], is_act_continuous)
        agent_action.append(action_)
    # print("agent_action: ", type(agent_action), len(agent_action[0]))  # list [[int]] ,38
    # print("agent_action: ", agent_action, sum(agent_action[0]) == 1)
    # print('\n')
